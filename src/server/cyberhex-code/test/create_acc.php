...
try {
    ...
    if ($fn === 'getCreateArgs') {
        ...
        $stmt = $conn->prepare("INSERT INTO users (username,user_hex_id, credential_id, public_key, counter) VALUES (?,?, ?, ?, ?)");
		//var_dump($data);
        $stmt->execute([$data->userName,$userId, $data->credentialId, $data->credentialPublicKey, $data->signatureCounter]);
